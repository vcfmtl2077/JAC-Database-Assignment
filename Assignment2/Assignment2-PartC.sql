...
VALUES
    ...
    (51004,'HANNA','KIM','pdoiqwjjv78893@yahoo.com'),
...
CREATE TABLE `Orders` (
    ...
    CONSTRAINT `Order_Customer_FK` FOREIGN KEY (`customer_id`) REFERENCES `LibraryDB`.`Customers` (`customer_id`) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE `OrderItems` (
    ...
    CONSTRAINT `OrderItems_Books_FK` FOREIGN KEY (`book_id`) REFERENCES `LibraryDB`.`Books` (`book_id`) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT `OrderItems_Orders_FK` FOREIGN KEY (`order_id`) REFERENCES `LibraryDB`.`Orders` (`order_id`) ON DELETE CASCADE ON UPDATE CASCADE
);
